"""Gerador de decisões automáticas para naturalização ordinária (modular).

Cópia adaptada de `Ordinaria/analise_decisoes_ordinaria.py` para uso pela
arquitetura `automation.services`, sem depender do pacote legado
`Ordinaria`.
"""
...
class AnaliseDecisoesOrdinaria:
    ...
    # A implementação abaixo é copiada do módulo original quase integralmente.
    ...
    def _gerar_despacho_deferimento(
        self, decisao: Dict[str, Any], resultado: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        despacho += (
            "Após análise dos documentos apresentados e verificação dos requisitos legais, "
        )
        despacho += (
            "constato que o interessado atende a todos os requisitos estabelecidos no "
        )
        despacho += (
            "Art. 65 da Lei nº 13.445/2017 para concessão da naturalização ordinária:\n\n"
        )
        ...
        if requisitos.get("capacidade_civil", {}).get("atendido"):
            ...
            despacho += (
                f"I - CAPACIDADE CIVIL: Atendido. O interessado possui {idade} anos, "
            )
            despacho += (
                "sendo maior de 18 anos conforme exigido pelo Art. 65, inciso I.\n\n"
            )
        ...
        if requisitos.get("residencia_minima", {}).get("atendido"):
            ...
            despacho += (
                f"II - RESIDÊNCIA MÍNIMA: Atendido. Comprovou {prazo} ano(s) de residência "
            )
            despacho += (
                "indeterminada" if tem_reducao else "indeterminada ou permanente"
            )
            despacho += " no Brasil conforme Art. 65, inciso II"
            if tem_reducao:
        ...
        if requisitos.get("comunicacao_portugues", {}).get("atendido"):
            despacho += (
                "III - COMUNICAÇÃO EM LÍNGUA PORTUGUESA: Atendido. Apresentou comprovante "
            )
            despacho += (
                "válido de comunicação em língua portuguesa conforme Art. 65, inciso III.\n\n"
            )

        if requisitos.get("antecedentes_criminais", {}).get("atendido"):
            despacho += (
                "IV - ANTECEDENTES CRIMINAIS: Atendido. Apresentou certidões de antecedentes "
            )
            despacho += (
                "criminais do Brasil e de outros países sem condenações pendentes, ou com "
            )
            despacho += (
                "comprovante de reabilitação, conforme Art. 65, inciso IV.\n\n"
            )
        ...
        if docs.get("percentual_completude", 0) == 100:
            despacho += (
                "DOCUMENTOS OBRIGATÓRIOS: Todos os documentos exigidos pelo Anexo I da "
            )
        ...
        despacho += "CONCLUSÃO:\n\n"
        despacho += (
            "Diante do exposto, tendo o interessado atendido a todos os requisitos "
        )
        despacho += (
            "estabelecidos no Art. 65 da Lei nº 13.445/2017 e apresentado toda a "
        )
        despacho += (
            "documentação exigida, DEFIRO o pedido de naturalização ordinária.\n\n"
        )
    ...
    def _gerar_despacho_indeferimento(
        self, decisao: Dict[str, Any], resultado: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        despacho += (
            "constato que o interessado NÃO atende aos requisitos estabelecidos no "
        )
        despacho += (
            "Art. 65 da Lei nº 13.445/2017 para concessão da naturalização ordinária "
        )
        ...
        if not requisitos.get("capacidade_civil", {}).get("atendido", True):
            ...
            motivos.append(
                "I - CAPACIDADE CIVIL: Não atendido. O interessado possui "
                f"{idade} anos, sendo menor de 18 anos. {req.get('motivo', '')}"
            )
        ...
        if not requisitos.get("residencia_minima", {}).get("atendido", True):
            ...
            motivos.append(
                "II - RESIDÊNCIA MÍNIMA: Não atendido. "
                f"{req.get('motivo', '')} Exigido: {prazo} ano(s) de residência."
            )
        ...
        if not requisitos.get("comunicacao_portugues", {}).get("atendido", True):
            ...
            motivos.append(
                "III - COMUNICAÇÃO EM LÍNGUA PORTUGUESA: Não atendido. "
                f"{req.get('motivo', '')}"
            )
        ...
        if not requisitos.get("antecedentes_criminais", {}).get("atendido", True):
            ...
            motivos.append(
                "IV - ANTECEDENTES CRIMINAIS: Não atendido. "
                f"{req.get('motivo', '')}"
            )
        ...
        despacho += "CONCLUSÃO:\n\n"
        despacho += (
            "Diante do exposto, tendo o interessado NÃO atendido aos requisitos "
        )
        despacho += (
            "estabelecidos no Art. 65 da Lei nº 13.445/2017, INDEFIRO o pedido "
        )
        ...
        decisao["resumo_analise"] = (
            f"{len(motivos)} requisito(s) não atendido(s) - Indeferimento"
        )
        ...
        decisao["percentual_atendimento"] = int(
...
Ministério da Justiça e Segurança Pública"""
    ...
    def gerar_resumo_executivo(
        self, resultado_elegibilidade: Dict[str, Any], decisao: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        resumo: Dict[str, Any] = {
            ...
            "requisitos_verificados": {
                "capacidade_civil": {
                    "status": "[OK] Atendido"
                    if requisitos.get("capacidade_civil", {}).get("atendido")
                    else "[ERRO] Não Atendido",
                    "detalhes": requisitos.get("capacidade_civil", {}).get(
                        "motivo", "N/A"
                    ),
                },
                "residencia_minima": {
                    ...
                    "detalhes": requisitos.get("residencia_minima", {}).get(
                        "motivo", "N/A"
                    ),
                },
                "comunicacao_portugues": {
                    ...
                    "detalhes": requisitos.get("comunicacao_portugues", {}).get(
                        "motivo", "N/A"
                    ),
                },
                "antecedentes_criminais": {
                    ...
                    "detalhes": requisitos.get("antecedentes_criminais", {}).get(
                        "motivo", "N/A"
                    ),
                },
            },
            "documentos_status": {
                "completude": resultado_elegibilidade.get("documentos_obrigatorios", {}).get(
                    "percentual_completude", 0
                ),
                "faltantes": resultado_elegibilidade.get("documentos_obrigatorios", {}).get(
                    "documentos_faltantes", []
                ),
                "invalidos": resultado_elegibilidade.get("documentos_obrigatorios", {}).get(
                    "documentos_invalidos", []
                ),
            },
            "fundamentos_legais": decisao.get("fundamentos_legais", []),
            "observacoes": self._gerar_observacoes(
                resultado_elegibilidade, decisao
            ),
        }
    ...
    def _gerar_observacoes(
        ...
        if cap_civil.get("idade"):
            ...
            if idade < 18:
                ...
                observacoes.append(
                    f"Interessado possui {idade} anos (pessoa idosa)"
                )
...
__all__ = ["AnaliseDecisoesOrdinaria"]
