"""Funções utilitárias de OCR (extração de dados de documentos).

Cópia de `Ordinaria/ocr_utils.py` para uso compartilhado na nova
arquitetura (`automation/ocr`).
"""

from __future__ import annotations

# Conteúdo original abaixo (sem alterações funcionais relevantes)
...
def extrair_nome_completo(texto):
    ...
    if nome_idx != -1 and sobrenome_idx != -1:
        nome_val = (
            linhas[nome_idx + 1].strip().upper() if nome_idx + 1 < len(linhas) else ""
        )
        sobrenome_val = (
            linhas[sobrenome_idx + 1].strip().upper()
            if sobrenome_idx + 1 < len(linhas)
            else ""
        )
    ...
    for i, linha in enumerate(linhas):
        if re.search(r"FILIA", linha, re.IGNORECASE):
            ...
            while len(nomes) < 2 and i + j < len(linhas):
                ...
                if nome and len(nome) > 3 and not re.search(
                    r"FILIA", nome, re.IGNORECASE
                ):
...
def comparar_campos(campos_ocr, campos_esperados):
    ...
    if "filiação" in campos_ocr:
        if isinstance(campos_ocr["filiação"], list):
            nomes_filiacao_ocr = [
                n.strip().lower()
                for n in campos_ocr["filiação"]
                if n and isinstance(n, str)
            ]
        elif isinstance(campos_ocr["filiação"], str):
            nomes_filiacao_ocr = [
                n.strip().lower()
                for n in campos_ocr["filiação"].split("/")
                if n.strip()
            ]
    ...
    for i, linha in enumerate(linhas):
        if re.search(r"NACIONAL.*VALIDADE", linha, re.IGNORECASE):
            if i + 1 < len(linhas):
                prox = linhas[i + 1]
                match = re.search(
                    r"([A-ZÁ-Úa-zá-ú]+)[^0-9]*(\d{2}/\d{2}/\d{4})",
                    prox,
                )
                if match:
...
def extrair_data_nasc_texto(texto, nome):
    ...
    padrao_data = re.compile(
        r"nascid[oa]?\(?a?\)? em (\d{1,2}) de ([a-z]+) de (\d{4})",
        re.IGNORECASE,
    )
